...
public class BoardController : MonoBehaviour
{
    ...
    private IEnumerator FillAfter()
    {
        yield return new WaitForSeconds(.1f);
        FillGapsWithNewItems();
    }

    private void FillGapsWithNewItems()
    {
        ...
        for (int x = 0; x < m_board.BoardSizeX; x++)
        {
            for (int y = 0; y < m_board.BoardSizeY; y++)
            {
                ...
                if (nitem != null)
                {
                    if (!count.ContainsKey(nitem.ItemType))
                    {
                        count[nitem.ItemType] = 0;
                    }
                    count[nitem.ItemType]++;
                }
            }
        }
        ...
        {
            int remain = key.Value % 3;
            if (remain == 0)
            {
                continue;
            }
            int howMuchToChange = remain;
            for (int x = 0; x < m_board.BoardSizeX; x++)
            {
                if (howMuchToChange == 0)
                {
                    break;
                }
                for (int y = 0; y < m_board.BoardSizeY; y++)
                {
                    ...
                    if (item != null && item.ItemType == key.Key)
                    {
        ...
        foreach (Cell c in cellsToChange)
        {
    ...
    public void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (hit.collider != null)
            {
                ...
                if (c1 != null) m_gameManager.MoveToBottomCells(c1);
            }
        }
