...
public class Board
{
    ...
    {
        int emptyCount = 0;
        Vector2Int? pos1= null, pos2= null, pos3 = null;
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                if (emptyCount == 3)
                {
                    ...
                    item.SetView();
                    item.SetViewRoot(m_root);
                    m_cells[pos1.Value.x, pos1.Value.y].Assign(item);m_cells[pos1.Value.x, pos1.Value.y].ApplyItemPosition(true);
                    m_cells[pos2.Value.x, pos2.Value.y].Assign(item);m_cells[pos2.Value.x, pos2.Value.y].ApplyItemPosition(true);
                    m_cells[pos3.Value.x, pos3.Value.y].Assign(item);m_cells[pos3.Value.x, pos3.Value.y].ApplyItemPosition(true);
                }
                else
                {
                    NormalItem item = new NormalItem();
